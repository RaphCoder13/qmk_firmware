...
void xcase_task(void) {
  if ((caps_word_enabled() || num_word_enabled() || x_case_enabled()) && timer_expired(timer_read(), idle_timer)) {
        disable_caps_word();
        disable_xcase();
        disable_num_word();
  }
...
bool process_case_modes(uint16_t keycode, const keyrecord_t *record) {
#if CAPS_WORD_IDLE_TIMEOUT > 0
    if(caps_word_on || xcase_state || num_word_on)
    ...
    if(get_mods() == MOD_MASK_SHIFT) {
        enable_caps_word(); // enable caps_word();
    }
#endif
