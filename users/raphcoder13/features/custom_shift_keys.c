    ...
    if ((mods | get_weak_mods()) & MOD_MASK_SHIFT) {  // Shift is held.
      ...
      for (int i = 0; i < NUM_CUSTOM_SHIFT_KEYS; ++i) {
        if (keycode == custom_shift_keys[i].keycode) {
          ...
          // We want to invert the case while in caps words
            if((is_caps_word_on() || host_keyboard_led_state().caps_lock) && ((mods | get_weak_mods() | get_oneshot_mods()) & MOD_MASK_SHIFT)) {
                if (IS_QK_MODS(registered_keycode) &&  // Should keycode be shifted?
                    (!custom_shift_keys[i].invert_on_shift) &&
                    (QK_MODS_GET_MODS(registered_keycode) & MOD_LSFT) != 0) {
                        // uprintf("In caps with shift and shifted target keycode\n");
                ...
                set_mods(mods);
                        ...
                        // uprintf("In caps with shift and NOT shifted target keycode\n");
                del_mods(MOD_MASK_SHIFT);
                del_weak_mods(MOD_MASK_SHIFT);
#ifndef NO_ACTION_ONESHOT
                del_oneshot_mods(MOD_MASK_SHIFT);
            ...
            } else if (IS_QK_MODS(registered_keycode) &&  // Should keycode be shifted?
              (QK_MODS_GET_MODS(registered_keycode) & MOD_LSFT) != 0) {
                        // uprintf("NOT In caps with shift and shifted target keycode\n");
                register_code16(registered_keycode);  // If so, press it directly.
            } else {
                // Otherwise cancel shift mods, press the key, and restore mods.
                        // uprintf("NOT In caps with shift and NOT shifted target keycode\n");
                del_weak_mods(MOD_MASK_SHIFT);
#ifndef NO_ACTION_ONESHOT
                del_oneshot_mods(MOD_MASK_SHIFT);
#endif  // NO_ACTION_ONESHOT
